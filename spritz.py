...
def print_word(word, orp_config, max_len, centering=False):
    ...
    if centering:
        print_string = (" " * (prefix + (cols - max_len) // 2)) + color_orp_char(word, orp) + (" " * postfix)
    else:
...
def spritz(wpm, reading, centering=False):
    """"Spritz" the ``reading``.
    ...
    for word in reading:
        ...
        print_word(word, orp_config, max_length, centering)
